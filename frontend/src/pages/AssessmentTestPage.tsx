...
import { ASSESSMENT_QUESTIONS, ALL_ASSESSMENT_CATEGORIES, useUpdateAssessmentResult } from '../hooks/useQueries';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Progress } from '@/components/ui/progress';
import { Badge } from '@/components/ui/badge';
import { ArrowLeft, ArrowRight, CheckCircle2, Loader2, Award, Trophy } from 'lucide-react';
...
export default function AssessmentTestPage() {
  ...
  const assessmentId = decodeURIComponent(pathParts[pathParts.length - 1] || '');

  const category = ALL_ASSESSMENT_CATEGORIES.find((c) => c.id === assessmentId);
  const questions = ASSESSMENT_QUESTIONS[assessmentId] || ASSESSMENT_QUESTIONS['programming'];
  ...
  const { mutateAsync: saveResult, isPending: isSaving } = useUpdateAssessmentResult();
  ...
  const getScoreLevel = (s: number): { label: string; color: string; bg: string } => {
    if (s >= 80) return { label: 'Expert', color: 'text-green-700', bg: 'bg-green-100' };
    if (s >= 60) return { label: 'Proficient', color: 'text-blue-700', bg: 'bg-blue-100' };
    if (s >= 40) return { label: 'Developing', color: 'text-yellow-700', bg: 'bg-yellow-100' };
            ...
            <CardContent className="p-8 text-center space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4 py-4 border-y border-border">
                <div className="text-center">
                  <p className="font-bold text-lg text-foreground">{totalQuestions}</p>
          ...
          <Card className="border-2 border-border shadow-sm animate-fade-in">
            <CardContent className="p-8 text-center space-y-6">
              ...
              <div className="bg-primary/5 rounded-2xl p-6 border border-primary/20">
                <p className="text-5xl font-display font-bold text-primary mb-2">{score}%</p>
                <Badge
                  variant="secondary"
                ...
                <p className="text-xs text-muted-foreground mt-3">
                  You answered {Math.round((score / 100) * totalQuestions)} out of {totalQuestions}{' '}
                  questions correctly
                </p>
              </div>
              ...
              <div className="text-left space-y-3">
                ...
                {[
                  { label: 'Expert (80–100%)', range: [80, 100], color: 'bg-green-500' },
                  { label: 'Proficient (60–79%)', range: [60, 79], color: 'bg-blue-500' },
                  { label: 'Developing (40–59%)', range: [40, 59], color: 'bg-yellow-500' },
                  { label: 'Beginner (0–39%)', range: [0, 39], color: 'bg-orange-500' },
                  ...
                  <div key={label} className="flex items-center gap-3">
                    ...
                    >
                      {label}
                      {score >= range[0] && score <= range[1] && ' ← Your level'}
                    </span>
                  </div>
                ))}
              </div>
                ...
                >
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
                      selectedAnswer === idx ? 'border-primary bg-primary' : 'border-muted-foreground/40'
                    }`}
                  >
